...
func TestHtmlHandleAlbum(t *testing.T) {
	t.Run("Positive", func(t *testing.T) {
		...
		sched := service.NewScheduler("y9FYFBmQyvdzYBY7", &data)
...
func TestHtmlHandlePair(t *testing.T) {
	t.Run("Positive", func(t *testing.T) {
		...
		stor := storage.NewMock()
		data := database.NewMem()
		sched := service.NewScheduler("9zYp2QUKJx8nsuKX", &data)
		serv := service.NewService(&comp, &stor, &data, &data, &sched)
	...
	t.Run("Negative", func(t *testing.T) {
		...
		stor := storage.NewMock()
		data := database.NewMem()
		sched := service.NewScheduler("NDUAttuKk2NaJAwq", &data)
		serv := service.NewService(&comp, &stor, &data, &data, &sched)
		html, err := newHtml(&serv)
		if err != nil {
			t.Error(err)
...
func TestHtmlHandleTop(t *testing.T) {
	t.Run("Positive", func(t *testing.T) {
		...
		stor := storage.NewMock()
		data := database.NewMem()
		sched := service.NewScheduler("zfAL4ZAqQ84kQSHf", &data)
		serv := service.NewService(&comp, &stor, &data, &data, &sched)
		contr := newController(&serv)
		html, err := newHtml(&serv)
		if err != nil {
	...
	t.Run("Negative", func(t *testing.T) {
		...
		stor := storage.NewMock()
		data := database.NewMem()
		sched := service.NewScheduler("WDsKDNfNM4Bt7UuB", &data)
		serv := service.NewService(&comp, &stor, &data, &data, &sched)
		html, err := newHtml(&serv)
		if err != nil {
			t.Error(err)
