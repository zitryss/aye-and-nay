...
func init() {
	...
	viper.Set("middleware.session.idLength", 16)
	viper.Set("middleware.session.timeToLive", "500ms")
	viper.Set("middleware.session.cleanupInterval", "500ms")
	...
	viper.Set("service.numberOfWorkers", 2)
	viper.Set("shortpixel.connections", 2)
